...
export default function createGraphqlSchema(source, destPath, options = {}, regenerateResolver = null) {
  return Promise.resolve(source).then(graphqlMetadata => {
    ...
    Object.keys(graphqlMetadata).forEach(k => {
      ...
      if (!type.fields._id && type.table) {
        type.fields = { _id: "MongoId", ...type.fields };
      }
      if (type.relationships) {
        Object.keys(type.relationships).forEach(k => {
          ...
          let fkArray = ["StringArray", "MongoIdArray", "IntArray", "FloatArray"].includes(fkFieldId);
    ...
    modules.forEach(objectToCreate => {
      if (regenerateResolver && regenerateResolver !== objectToCreate.__name) {
        return;
      }
